...
type BlogServiceClient interface {
	...
	ListAllBlogs(ctx context.Context, in *emptypb.Empty, opts ...grpc.CallOption) (BlogService_ListAllBlogsClient, error)
}
...
func (c *blogServiceClient) ListAllBlogs(ctx context.Context, in *emptypb.Empty, opts ...grpc.CallOption) (BlogService_ListAllBlogsClient, error) {
	stream, err := c.cc.NewStream(ctx, &BlogService_ServiceDesc.Streams[0], "/blog.BlogService/ListAllBlogs", opts...)
	if err != nil {
		return nil, err
	}
	x := &blogServiceListAllBlogsClient{stream}
...
type BlogService_ListAllBlogsClient interface {
	Recv() (*Blog, error)
	grpc.ClientStream
}

type blogServiceListAllBlogsClient struct {
	grpc.ClientStream
}

func (x *blogServiceListAllBlogsClient) Recv() (*Blog, error) {
	m := new(Blog)
...
type BlogServiceServer interface {
	...
	ListAllBlogs(*emptypb.Empty, BlogService_ListAllBlogsServer) error
...
func (UnimplementedBlogServiceServer) ListAllBlogs(*emptypb.Empty, BlogService_ListAllBlogsServer) error {
	return status.Errorf(codes.Unimplemented, "method ListAllBlogs not implemented")
}
...
func _BlogService_ListAllBlogs_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(emptypb.Empty)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(BlogServiceServer).ListAllBlogs(m, &blogServiceListAllBlogsServer{stream})
}

type BlogService_ListAllBlogsServer interface {
	Send(*Blog) error
	grpc.ServerStream
}

type blogServiceListAllBlogsServer struct {
	grpc.ServerStream
}

func (x *blogServiceListAllBlogsServer) Send(m *Blog) error {
...
var BlogService_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		{
			StreamName:    "ListAllBlogs",
			Handler:       _BlogService_ListAllBlogs_Handler,
			ServerStreams: true,
		},
	},
	Metadata: "blog.proto",
